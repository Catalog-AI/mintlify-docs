...
```json
{
  ...
  "regenerate_description_html": false,
  "country_code": "us",
  "force_extraction": false,
  "populate_description_html_from_html": false
}
```
...
- Execution ID format: `extract-urls-{uuid}` for direct URLs mode

#### Enable Flags and Response Schema

The `/v2/extract` endpoint maintains a consistent response schema regardless of `enable_*` flag values. All fields are always present, but will be `null` when the corresponding flag is `false` or when data is unavailable.

**Field Mappings:**

| Flag | Affected Fields | Behavior When `false` |
|------|----------------|----------------------|
| `enable_reviews` | `reviews` | Always `null` |
| `enable_enrichment` | `attributes`, `product_type`, `google_product_category_id`, `google_product_category_path` | All set to `null` |
| `enable_image_tags` | `images[].attributes` | Each image's `attributes` field is `null` |
| `enable_brand_pdp` | `brand_pdp` | Always `null` |
| `enable_similar_products` | `similar_products` | Always `null` |
| `enable_reddit_insights` | `reddit_insights` | Always `null` |

**Example Response Comparison:**

With `enable_reviews: false`:
```json
{
  "id": "prod_123",
  "title": "Product Title",
...
With `enable_reviews: true` and `enable_enrichment: true`:
```json
{
  "id": "prod_123",
  "title": "Product Title",
...
**Key Difference:**
- `/api/products` returns all products immediately (synchronous, blocking)
- `/v2/extract` returns an execution ID and requires polling for results (asynchronous, non-blocking)
- `/api/products` supports up to 100 URLs per request
- `/v2/extract` supports up to 1000 URLs per request
...
**v2 Status Response (when completed):**
```json
{
  "execution_id": "extract-urls-965b1912-6af0-4ed8-b7e3-184b85e788b7",
  "status": "completed",
...
**Key Changes:**
- Response structure changed from array of `{url, product, success}` objects to flat `data` array of products
- Failed products are excluded from `data` array (check `meta.result.products_failed` for failures)
- Price format changed from `price_amount`/`price_currency` to `price` JSON object
- `enable_enrichment` default changed from `true` to `false` in v2
- HTTP status code changed to `202 Accepted` for POST (was `200 OK`)
- Requires polling `GET /v2/extract/{execution_id}` to retrieve results
- Support for pagination when retrieving results (use `page` and `page_size` query parameters)
- URL limit increased from 100 (API) to 1000 (v2) per request
- Execution ID format: `extract-urls-{uuid}` for direct URLs mode
...
The `/v2/agentic-search` endpoint uses the same product processing pipeline as `/v2/extract`, so the response schema behavior matches extract endpoints.
...
**Note**: Agentic search currently only supports `enable_enrichment`. Other flags (reviews, image tags, etc.) are not available for agentic search results.

**Example Response:**

With `enable_enrichment: false`:
```json
{
  "data": [
    {
...
All endpoints that support `enable_*` flags maintain a **consistent response schema**. Fields are always present in the response, but will be `null` (or empty objects/arrays) when the corresponding flag is `false` or when data is unavailable.
