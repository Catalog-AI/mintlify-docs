...
**Note:** In v2, both direct URLs and vendor-based extraction use the same endpoint (`/v2/extract`). The input source is determined by the request body: use `urls` for direct product URLs, or `vendor` for extracting from a vendor domain.
...
v2 introduces new endpoints to manage asynchronous search executions:
