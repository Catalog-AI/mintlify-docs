...
The Extract endpoints have undergone significant changes in v2. The `/v1/products` endpoint has been replaced with `/v2/extract`, which offers two distinct modes of operation.
...
POST /v1/products
GET /v1/products
GET /v1/products/{execution_id}
DELETE /v1/products/{execution_id}
```

**v2:**
```
POST /v2/extract (Direct URLs mode)
POST /v2/extract (Vendor-based mode)
GET /v2/extract
GET /v2/extract/{execution_id}
DELETE /v2/extract/{execution_id}
```

#### 2. **New Extraction Modes**

v2 introduces two extraction modes:

**Direct URLs Mode** (Similar to v1 `/v1/products`):
- Use when you have specific product URLs
- Post to `/v2/extract` with `urls` array in the request body
- Works the same way as v1 but with updated response format

**Vendor-Based Mode** (New in v2):
...
```json
{
  ...
  "country_code": "us",
  // ... same optional parameters as direct URLs mode
}
```
...
- Vendor-based mode adds `vendor`, `max_products`, and `crawl_id` parameters
...
1. **Update endpoint paths:** Change `/v1/products` to `/v2/extract`
2. **Update execution ID handling:** Change format from `products-batch-{uuid}` to `extract-urls-{uuid}` or `extract-{vendor}-{uuid}`
3. **Update response parsing:** Change `results.products` to `data`, `results.meta` to `meta.result`
4. **Update query parameters:** Change `limit` to `page_size` in status endpoint (or continue using `limit` if supported for backward compatibility)
5. **Update default values:** Set `enable_enrichment` explicitly if you were relying on v1's default of `true`
6. **Consider vendor-based mode:** If you're extracting all products from a vendor, consider using the new vendor-based mode with `crawl_id` for better workflow
