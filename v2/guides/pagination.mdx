...
<Note>
  In v2, all endpoints consistently return data in a `data` array and use `page_size` for pagination parameters. The pagination object structure is consistent across all endpoints.
</Note>
...
<Tabs>
  <Tab title="Python">
    ...
    **Endpoints:** `/v2/listings`, `/v2/collections`, `/v2/agentic-search`
    
    Include `page` and `page_size` in the request body. Response includes a `pagination` object.
    ...
    **Endpoints:** `/v2/products/{execution_id}`
...
<Note>
  **Consistent Structure in v2:**
  - All endpoints return data in a `data` array
  - All endpoints use `pagination` object (not `meta`)
  - All endpoints use `page` in response (not `current_page`)
  - All endpoints use `page_size` consistently
</Note>
