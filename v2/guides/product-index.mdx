...
<Note>
This guide covers multiple approaches: browsing existing vendors via [Get Vendors](/v2/api-reference/endpoints/discover/get-vendors), discovering new product listings via [Start Crawl](/v2/api-reference/endpoints/discover/start-crawl), and extracting products via [Extract](/v2/api-reference/endpoints/extract/extract) using either direct URLs or a vendor domain.
</Note>
...
### Step 2: Extract Products from Vendor

After crawling (or while a crawl is running), use the **Extract** endpoint (`POST /v2/extract`) with a vendor domain to enhance products with full details, AI enrichment, reviews, and image tags from all discovered listings.
...
### Step 3: Add Missing Products with Direct URLs

Use the **Extract** endpoint (`POST /v2/extract`) with direct URLs to add any products whose listings weren't discovered during crawl, or to refresh specific products from your own URL list.
...
**Use direct URLs for targeted updates:** For specific products that need frequent updates (e.g., featured items), use the Extract endpoint with direct URLs to refresh them more frequently than the full extract cycle.
