...
<Note>
This guide covers both approaches: discovering products via [Get Vendors](/v2/api-reference/endpoints/discover/get-vendors) and collections, or indexing a self-selected set of URLs via [Get Products](/v2/api-reference/endpoints/extract/retrieve-products).
</Note>
...
Once you have a list of product URLs, use the **Get Products** endpoint (`POST /v2/products`) to hydrate many URLs in a single request (1-1000 URLs per request).

**Typical flow:**

1. Pull a batch of URLs from your queue or database
2. Call `POST /v2/products` with up to 1000 URLs per request
3. Receive an `execution_id` and poll the [Get Execution Status](/v2/api-reference/endpoints/extract/get-execution-status) endpoint
...
Use the **Get Products** endpoint (`POST /v2/products`) for bulk hydration or scheduled backfills (supports 1-1000 URLs per request).
...
2. Call `POST /v2/products` with:
   - `urls`: your batch of URLs (1-1000 per request)
   - `enable_enrichment`: `false` if you want AI-enriched attributes (default: `false`)
...
<CardGroup cols={2}>
  ...
  <Card title="Get Products" icon="box" href="/v2/api-reference/endpoints/extract/retrieve-products">
