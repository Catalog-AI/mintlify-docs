...
<Note>
This guide covers multiple approaches: browsing existing vendors via [`GET /v2/vendors`](/v2/api-reference/endpoints/discover/get-vendors), discovering new product listings via [`POST /v2/crawl`](/v2/api-reference/endpoints/discover/crawl), and extracting products via [`POST /v2/extract`](/v2/api-reference/endpoints/extract/extract) using either URLs or a vendor.
</Note>
...
Use [`GET /v2/vendors`](/v2/api-reference/endpoints/discover/get-vendors) to discover which vendors are available and how many products they have.
...
If your chosen vendors support collections, you can use [`POST /v2/collections`](/v2/api-reference/endpoints/discover/get-collections) to:
...
Use [`POST /v2/listings`](/v2/api-reference/endpoints/discover/get-listings) to page through all products for a vendor or collection. As you page through:
...
Once you have a list of product URLs, use [`POST /v2/extract`](/v2/api-reference/endpoints/extract/extract) to hydrate many URLs in a single request (up to 1000 URLs per request).
...
Use [`POST /v2/crawl`](/v2/api-reference/endpoints/discover/crawl) to discover all product listings from a vendor website. This endpoint:
...
3. Poll [`GET /v2/crawl/{execution_id}`](/v2/api-reference/endpoints/discover/get-crawl-status) to check progress:
...
After crawling (or while a crawl is running), use [`POST /v2/extract`](/v2/api-reference/endpoints/extract/extract) with a vendor to enhance products with full details, AI enrichment, reviews, and image tags from all discovered listings.
...
Use [`POST /v2/extract`](/v2/api-reference/endpoints/extract/extract) with URLs to add any products whose listings weren't discovered during crawl, or to refresh specific products from your own URL list.
...
4. Poll [`GET /v2/extract/{execution_id}`](/v2/api-reference/endpoints/extract/get-extract-status) to check progress
