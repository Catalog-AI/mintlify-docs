...
Once you have a list of product URLs, use the **Extract** endpoint (`POST /v2/extract`) to hydrate many URLs in a single request (up to 1000 URLs per request).
...
After crawling (or while a crawl is running), use the **Extract** endpoint (`POST /v2/extract`) with a vendor to enhance products with full details, AI enrichment, reviews, and image tags from all discovered listings.
...
### Step 3: Add Missing Products with URLs

Use the **Extract** endpoint (`POST /v2/extract`) with URLs to add any products whose listings weren't discovered during crawl, or to refresh specific products from your own URL list.
...
- Use Extract with URLs for targeted updates of specific items
...
**Use URLs for targeted updates:** For specific products that need frequent updates (e.g., featured items), use the Extract endpoint with URLs to refresh them more frequently than the full extract cycle.
