...
Retrieve the status, progress, and results for an asynchronous extraction job started via the [Extract Direct URLs](/v2/api-reference/endpoints/extract/extract-direct-urls) or [Extract Vendor-Based](/v2/api-reference/endpoints/extract/extract-vendor-based) endpoints.
...
<Note>
**Supported Execution Types:** This endpoint works for both extraction modes:
- **Direct URLs mode:** Execution IDs formatted as `extract-urls-{uuid}`
- **Vendor-based mode:** Execution IDs formatted as `extract-{vendor}-{uuid}` (e.g., `extract-nike-com-51d87084`)

Both types of executions use the same status endpoint and return the same response structure.

**Authentication Required:** This endpoint requires a valid API key. Only executions belonging to your organization are accessible.
</Note>
...
<ParamField path="execution_id" type="string" required>
  The execution ID returned from the [Extract Direct URLs](/v2/api-reference/endpoints/extract/extract-direct-urls) or [Extract Vendor-Based](/v2/api-reference/endpoints/extract/extract-vendor-based) POST endpoint
  
  **Formats:**
  - `extract-urls-{uuid}` - Direct URLs mode (e.g., `extract-urls-965b1912-6af0-4ed8-b7e3-184b85e788b7`)
