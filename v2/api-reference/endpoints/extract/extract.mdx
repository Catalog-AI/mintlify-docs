...
description: "Async extraction of product data from URLs or a vendor. Returns immediately with an execution_id to check status and retrieve results."
---
...
<ParamField body="vendor" type="string">
  ...
  **Important:** This vendor must have been crawled previously using [`POST /v2/crawl`](/v2/api-reference/endpoints/crawl/crawl), or you must provide a `crawl_id` to wait for an active crawl to complete. Extraction requires product listings to have been discovered via crawl.
</ParamField>
...
<ParamField body="max_products" type="number">
...
<ParamField body="crawl_id" type="string">
  ...
  **Note:** If you haven't started a crawl yet, use [`POST /v2/crawl`](/v2/api-reference/endpoints/crawl/crawl) first, then use the returned `execution_id` as the `crawl_id` parameter here.
</ParamField>
...
1. **Crawl the Domain (if not already done):** First, use [`POST /v2/crawl`](/v2/api-reference/endpoints/crawl/crawl) to discover product listings from the domain. Wait for the crawl to complete or use the `crawl_id` parameter in step 2.
