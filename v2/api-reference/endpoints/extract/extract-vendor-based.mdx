...
<Note>
**Crawl Requirement:** Product extraction can only be performed on websites whose products are currently being crawled or have already been crawled. Before starting an extraction, you must first run a [crawl](/v2/api-reference/endpoints/discover/start-crawl) to discover and index the domain's products. The extraction will use the product listings discovered during the crawl process.
...
<ParamField body="vendor" type="string" required>
  ...
  **Important:** This vendor domain must have been crawled previously using the [Start Crawl](/v2/api-reference/endpoints/discover/start-crawl) endpoint, or you must provide a `crawl_id` to wait for an active crawl to complete. Extraction requires product listings to have been discovered via crawl.
</ParamField>
...
<ParamField body="crawl_id" type="string" optional>
  ...
  **Note:** If you haven't started a crawl yet, use the [Start Crawl](/v2/api-reference/endpoints/discover/start-crawl) endpoint first, then use the returned `execution_id` as the `crawl_id` parameter here.
</ParamField>
...
1. **Crawl the Domain (if not already done):** First, use the [Start Crawl](/v2/api-reference/endpoints/discover/start-crawl) endpoint to discover products from the domain. Wait for the crawl to complete or use the `crawl_id` parameter in step 2.
