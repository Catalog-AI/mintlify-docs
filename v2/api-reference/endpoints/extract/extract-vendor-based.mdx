...
<Note>
**Crawl Requirement:** Product extraction can only be performed on websites whose product listings are currently being crawled or have already been crawled. Before starting an extraction, you must first run a [crawl](/v2/api-reference/endpoints/discover/start-crawl) to discover and index the domain's product listings. The extraction will use the product listings discovered during the crawl process.
...
- **Waiting for Crawl:** You can optionally provide a `crawl_id` to wait for a crawl execution to complete before starting extraction. This is useful when you want to ensure product listings are discovered before extraction begins.
...
1. **Crawl the Domain (if not already done):** First, use the [Start Crawl](/v2/api-reference/endpoints/discover/start-crawl) endpoint to discover product listings from the domain. Wait for the crawl to complete or use the `crawl_id` parameter in step 2.
