...
description: "Start asynchronous extraction of product URLs to retrieve detailed product information"
---

<Warning>
**Deprecated:** This endpoint has been replaced with the [Extract Direct URLs](/v2/api-reference/endpoints/extract/extract-direct-urls) endpoint. Use `POST /v2/extract` with a `urls` array instead. This page is maintained for backward compatibility reference.
</Warning>

Start asynchronous extraction for multiple product URLs. This endpoint returns immediately with an `execution_id` that you can use to check status and retrieve results. Supports processing 1 to 1000 URLs per request.
...
**Migration:** This endpoint is now available as the "Direct URLs" mode of the Extract endpoint. See [Extract Direct URLs](/v2/api-reference/endpoints/extract/extract-direct-urls) for the current implementation.
</Note>
...
<ResponseField name="execution_id" type="string">
  ...
  **Format:** `extract-urls-{uuid}` (when using the new Extract Direct URLs endpoint)
</ResponseField>
...
1. Use the `execution_id` to poll the [Get Extract Execution Status](/v2/api-reference/endpoints/extract/get-extract-status) endpoint at `/v2/extract/{execution_id}`
...
```bash cURL
# Using the new Extract Direct URLs endpoint
...
// Start async extraction with direct URLs (using the new Extract endpoint)
...
console.log(`Extraction started: ${execution_id}, Credits used: ${meta.credits_used}`);

// Poll for results using the execution_id at /v2/extract/{execution_id}
...
print(f"Extraction started: {execution_id}, Credits used: {data['meta']['credits_used']}")
...
1. **Start Extraction:** Send a POST request to `/v2/extract` with your URLs array to start async extraction
2. **Get Execution ID:** Receive an `execution_id` immediately in the response (HTTP 202), format: `extract-urls-{uuid}`
3. **Check Status:** Poll the [Get Extract Execution Status](/v2/api-reference/endpoints/extract/get-extract-status) endpoint at `/v2/extract/{execution_id}` using the `execution_id`
4. **Retrieve Results:** When `status` is `"completed"`, results are available with pagination support

<Note>
**See [Extract Direct URLs](/v2/api-reference/endpoints/extract/extract-direct-urls) for the current implementation and full documentation.**
</Note>

## Features
- **Large-scale extraction** - Handles 1 to 1000 URLs per batch
- **Asynchronous execution** - Returns immediately with HTTP 202, extraction runs in background
- **Real-time progress tracking** - Monitor progress via extract execution status endpoint
...
<Warning>
**This endpoint is deprecated.** Please use the [Extract Direct URLs](/v2/api-reference/endpoints/extract/extract-direct-urls) endpoint instead, which provides the same functionality with an improved implementation.
</Warning>
