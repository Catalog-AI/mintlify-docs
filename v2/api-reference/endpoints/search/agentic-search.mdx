---
title: "Start Agentic Search"
api: "POST https://api.getcatalog.ai/v2/agentic-search"
description: "Start asynchronous AI-powered product search"
---

Start an asynchronous agentic search. This endpoint performs advanced product searches that reason through natural language queries and customer preferences. Returns immediately with an `execution_id` that you can use to check status and retrieve results.

<Tip>
**When to use:** Best for personalized shopping experiences where you have customer profile data and need comprehensive product information. For faster searches with up to 10 results, use [Agentic Search Mini](/v2/api-reference/endpoints/search/agentic-search-mini).
</Tip>

<Note>
**Async Processing:** This endpoint starts processing asynchronously and returns an `execution_id` immediately. Use the [Get Agentic Search Execution Status](/v2/api-reference/endpoints/search/get-agentic-search-status) endpoint to check progress and retrieve results when processing completes.

**View All Executions:** You can view all your agentic search executions using the [List Agentic Search Executions](/v2/api-reference/endpoints/search/list-agentic-search-executions) endpoint to see all search jobs, their statuses, and when they were created.

**Cancel Execution:** You can cancel a running agentic search execution using the [Cancel Agentic Search Execution](/v2/api-reference/endpoints/search/cancel-agentic-search-execution) endpoint if you need to stop a search that is currently processing.
</Note>
...
<ResponseField name="execution_id" type="string">
  Unique execution identifier for this search job. Use this ID with the [Get Agentic Search Execution Status](/v2/api-reference/endpoints/search/get-agentic-search-execution-status) endpoint to check progress and retrieve results.
...
The `/v2/agentic-search` endpoint maintains a consistent response schema regardless of `enable_*` flag values. All fields are always present in product objects returned in the `data` array, but will be `null` when the corresponding flag is `false`.
