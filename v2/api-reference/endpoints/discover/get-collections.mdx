...
api: "POST https://api.getcatalog.ai/v2/collections"
description: "Retrieve collections from a specific vendor with pagination"
---

Retrieve collections from a specific vendor with pagination. Collections are ordered by product count (highest first).

<Tip>
**When to use:** Browse vendor collections before fetching products. Use with [Get Product Listings](/v2/api-reference/endpoints/discover/get-listings) to get products from specific collections.
</Tip>
...
```bash cURL
curl -X POST https://api.getcatalog.ai/v2/collections \
...
const response = await fetch('https://api.getcatalog.ai/v2/collections', {
  method: 'POST',
  headers: {
