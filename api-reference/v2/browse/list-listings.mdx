...
Retrieve products from a vendor with optional collection filtering. Returns paginated results with metadata.

<Tip>
**When to use:** Browse or index all products from a vendor. For full product details, pass the URLs to [Get Products](/api-reference/endpoints/products/get-products).
</Tip>
    ...
    <ResponseField name="price" type="object" optional>
      ...
      <Expandable title="Price Object">
        ...
        <ResponseField name="current_value" type="number">
          Current price value (the price a shopper would pay now)
        </ResponseField>
        
        <ResponseField name="compare_at_value" type="number" optional>
          Original or reference price. When no separate reference price exists, this will match `current_value`. Example: if there is a sale, changing the price from 50 to 45, `compare_at_value` is 50 and `current_value` is 45.
        </ResponseField>
      </Expandable>
    </ResponseField>
...
console.log(`Found ${data.collection_products.length} listings`);
```
