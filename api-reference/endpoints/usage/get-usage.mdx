...
<ResponseField name="usage" type="array">
  ...
  <Expandable title="Usage Statistics">
    <ResponseField name="path" type="string">
      API endpoint path (e.g., "/v1/search", "/v1/product")
    </ResponseField>
...
```bash cURL
curl -X GET "https://api.getcatalog.ai/v1/usage?period=7d" \
  -H "x-api-key: YOUR_API_KEY"
```

```javascript JavaScript
const response = await fetch('https://api.getcatalog.ai/v1/usage?period=7d', {
  method: 'GET',
  headers: {
...
response = requests.get(
    'https://api.getcatalog.ai/v1/usage?period=7d',
    headers={
        'x-api-key': 'YOUR_API_KEY'
    }
)
