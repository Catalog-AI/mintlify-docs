...
api: "DELETE https://api.getcatalog.ai/v1/extract/{execution_id}"
...
<Note>
**Cancellation Requirements:** You can only cancel executions that are currently running. If an execution has already completed or failed, it cannot be canceled. Executions that are waiting for a crawl to complete (status "pending" with `waiting_for`) cannot be canceled until they start running. The endpoint will return an error if you attempt to cancel a non-running execution.
</Note>
...
```bash cURL
curl -X DELETE "https://api.getcatalog.ai/v1/extract/extract-nike-com-550e8400" \
...
const response = await fetch(
  `https://api.getcatalog.ai/v1/extract/${executionId}`,
  {
    method: 'DELETE',
    headers: {
