...
Retrieve the status and results for an asynchronous crawl job started via the [Start Crawl](/v1/api-reference/endpoints/crawl/start-crawl) endpoint.
...
**Cancel Running Executions:** If you need to stop a crawl that is currently running, use the [Cancel Crawl Execution](/v1/api-reference/endpoints/crawl/cancel-execution) endpoint to stop the execution.
</Note>
...
<ParamField path="execution_id" type="string" required>
  The execution ID returned from the [Start Crawl](/v1/api-reference/endpoints/crawl/start-crawl) POST endpoint
...
curl -X GET "https://api.getcatalog.ai/v1/crawl/crawl-example-com-a1b2c3d4" \
...
5. **Cancellation:** If a crawl is taking too long or you need to stop it, use the [Cancel Crawl Execution](/v1/api-reference/endpoints/crawl/cancel-execution) endpoint to stop running executions
