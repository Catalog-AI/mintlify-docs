...
**Cancel Running Executions:** If you need to stop a crawl that is currently running, use [`DELETE /v1/crawl/{execution_id}`](/v1/api-reference/endpoints/discover/cancel-crawl-execution) to stop the execution.
</Note>
...
<ParamField path="execution_id" type="string" required>
  The execution ID returned from [`POST /v1/crawl`](/v1/api-reference/endpoints/discover/crawl)
...
5. **Cancellation:** If a crawl is taking too long or you need to stop it, use [`DELETE /v1/crawl/{execution_id}`](/v1/api-reference/endpoints/discover/cancel-crawl-execution) to stop running executions
