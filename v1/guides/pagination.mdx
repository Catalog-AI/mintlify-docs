...
```json
{
  "products": [...],  // or "data", "listings" depending on endpoint
  "pagination": {
...
class ProductSearchPaginator:
    def __init__(self, api_key):
        ...
        self.base_url = 'https://api.getcatalog.ai/v1/agentic-search'
...
const InfiniteProductList = () => {
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (data.products.length === 0 || !data.pagination.has_next) {
        setHasMore(false);
      } else {
...
For GET endpoints like `/v1/vendors`, use query parameters:
...
For POST endpoints like `/v1/listings`, include pagination in the request body:
...
For the execution status endpoint (`/v1/products/{execution_id}`), use `limit` instead of `page_size`:
...
if (data.status === 'completed') {
  console.log(`Retrieved ${data.results.products.length} products`);
  console.log(`Page ${data.pagination.page} of ${data.pagination.total_pages}`);
}
```
...
data = response.json()
if data['status'] == 'completed':
  print(f"Retrieved {len(data['results']['products'])} products")
  print(f"Page {data['pagination']['page']} of {data['pagination']['total_pages']}")
```
...
<Note>
  Different endpoints may return data in different fields (`products`, `data`, `listings`). Always check the endpoint documentation for the specific response structure. The pagination object structure is consistent across all endpoints.
</Note>
