...
Build and maintain your own product index using Catalog's browse, crawl, extract, and product endpoints. Choose the approach that best fits your needs: browse existing vendors that Catalog has already discovered, or crawl and extract new vendors to ensure you have the latest data.

<Note>
This guide covers multiple approaches: browsing existing vendors via [Get Vendors](/v1/api-reference/endpoints/discover/get-vendors), discovering new products via [Start Crawl](/v1/api-reference/endpoints/discover/start-crawl), enhancing them with [Start Extract](/v1/api-reference/endpoints/extract/start-extract), and adding missing products via [Get Products](/v1/api-reference/endpoints/extract/retrieve-products).
</Note>
...
If your chosen vendors support collections, you can use the **Get Collections** endpoint (`POST /v1/collections`) to:
...
Use the **Get All Products** endpoint (`POST /v1/listings`) to page through all products for a vendor or collection. As you page through:
...
3. Receive an `execution_id` and poll the [Get Execution Status](/v1/api-reference/endpoints/extract/get-products-execution-status) endpoint
...
Use the **Start Crawl** endpoint (`POST /v1/crawl`) to discover all products from a vendor website. This endpoint:
...
3. Poll the [Get Crawl Execution Status](/v1/api-reference/endpoints/discover/get-crawl-status) endpoint to check progress:
...
### Step 2: Enhance with Extract

After crawling (or while a crawl is running), use the **Start Extract** endpoint (`POST /v1/extract`) to enhance products with full details, AI enrichment, reviews, and image tags.

**Typical flow:**

1. Start extraction by calling `POST /v1/extract` with the domain:
   ```json
   {
     "domain": "example.com",
...
4. Poll the [Get Execution Status](/v1/api-reference/endpoints/extract/get-extract-status) endpoint:
   - Check `status` and `meta.progress` for real-time progress
   - When `status === "completed"`, results are available with pagination
   - Use `page` and `limit` query parameters to retrieve results in chunks
...
Use the **Get Products** endpoint (`POST /v1/products`) to add any products that weren't discovered during crawl/extract, or to refresh specific products from your own URL list.
...
4. Poll the [Get Execution Status](/v1/api-reference/endpoints/extract/get-products-execution-status) endpoint to check progress
...
<CardGroup cols={2}>
  <Card title="Start Crawl" icon="spider" href="/v1/api-reference/endpoints/discover/start-crawl">
    Discover products from a vendor website
  </Card>
  <Card title="Start Extract" icon="wand-magic-sparkles" href="/v1/api-reference/endpoints/extract/start-extract">
    Enhance products with full details and AI enrichment
  </Card>
  <Card title="Get Products" icon="box" href="/v1/api-reference/endpoints/extract/retrieve-products">
    Bulk fetch product details by URLs
  </Card>
  <Card title="Get Vendors" icon="store" href="/v1/api-reference/endpoints/discover/get-vendors">
    Discover available vendors
  </Card>
  <Card title="Get Collections" icon="folder" href="/v1/api-reference/endpoints/discover/get-collections">
    Browse vendor collections
  </Card>
  <Card title="Get All Products" icon="list" href="/v1/api-reference/endpoints/discover/get-products">
