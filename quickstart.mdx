        ...
        const pollStatus = async () => {
          while (true) {
            const statusResponse = await fetch(
              `https://api.getcatalog.ai/v1/products/executions/${execution_id}`,
              {
                headers: {
                  'x-api-key': process.env.CATALOG_API_KEY
                }
  ...
  <Tab title="cURL">
    ...
    <Tabs>
      ...
      <Tab title="Get Products">
        ...
        curl -X GET "https://api.getcatalog.ai/v1/products/executions/EXECUTION_ID" \
          -H "x-api-key: ${CATALOG_API_KEY}"
        ```
      </Tab>
